    ...
    companion object {
        ...
        fun getStatus(context: Context, status: EnumValue): StatusResource? {
            return when (status) {
                ...
                WorkoutStatus.REJECTED -> {
                    StatusResource(R.color.on_primary, R.color.workout_reject, context.getString(R.string.rejected))
                }
                TeacherStatus.VERIFY -> {
                    StatusResource(R.color.color_secondary_variant, R.color.workout_verify, context.getString(R.string.pending_verification))
                }
                else -> {
                    null
                }
        ...
        fun getStudentStatus(context: Context, status: EnumValue?): StatusResource? {
            return when (status) {
                ...
                WorkoutStatus.REJECTED -> {
                    StatusResource(R.color.color_text_primary, 0, context.getString(R.string.rejected))
                }
                else -> {
                    null
                }
