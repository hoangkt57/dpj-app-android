...
class ChangePasswordViewModel(application: Application) : BaseViewModel(application) {
    ...
    fun isValidPassword(oldPass: String, newPass: String, newPassConfirm: String): Boolean {
        ...
        if (newPass != newPassConfirm) {
            status.postValue(PASSWORD_ERROR_NOT_EQUAL_CONFIRM)
            return false
        }

        if (newPass.length < 8) {
            status.postValue(PASSWORD_ERROR_LENGTH_8)
            return false
        }

        if (oldPass == newPass) {
            status.postValue(PASSWORD_ERROR_SAME_OLD_PASS)
            return false
        }

        if (newPass.matches(Regex("[0-9]+"))) {
            status.postValue(PASSWORD_ERROR_NEED_LETTER)
            return false
        }

        if (newPass.matches(Regex("^[a-zA-Z]*$"))) {
            status.postValue(PASSWORD_ERROR_NEED_NUMBER)
            return false
        }
        ...
        viewModelScope.launch(Dispatchers.Default) {
            try {
                ...
                if (!infoResponse.data?.userChangePassword?.asUserMutationErrors?.errors.isNullOrEmpty()) {
                    status.postValue(PASSWORD_ERROR_INVALID)
                    return@launch
                } else {
                    status.postValue(PASSWORD_ERROR_NONE)
                }
            } catch (e: Exception) {
